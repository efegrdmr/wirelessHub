...
{
    /* Block until WiFi acquires an IP address */
    ...
    while (true) {
        ...
        if (rcvd < (ssize_t)(sizeof(Header) + sizeof(DiscoverReplyPayload))) {
            ESP_LOGW(TAG, "No valid reply (rcvd=%d) — retrying in %d s...", (int)rcvd, DISCOVER_RETRY_SEC);
            vTaskDelay(pdMS_TO_TICKS(DISCOVER_RETRY_SEC * 1000));
            continue;
        }
        ...
        if (hdr->cmd_type != CMD_DISCOVER_REPLY) {
            ESP_LOGW(TAG, "Unexpected cmd_type=0x%02x — retrying in %d s...", hdr->cmd_type, DISCOVER_RETRY_SEC);
            vTaskDelay(pdMS_TO_TICKS(DISCOVER_RETRY_SEC * 1000));
            continue;
        }
...
{    s_hub_event_group = xEventGroupCreate();    xTaskCreate(hub_discover_task, "hub_discover", 4096, NULL, 5, NULL);
}
